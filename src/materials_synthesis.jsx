...
const T = {
  ...
  syn_main: "#0d3339", syn_cvd: "#398a9a", syn_pvd: "#378695", syn_sol: "#56a3b3",
  syn_spin: "#134650", syn_ald: "#347d8b", syn_mbe: "#68adbb", syn_hydro: "#4f9faf"
};
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 500, color, fontFamily: "monospace" }}>{fmt(value)}{unit || ""}</span>
      </div>
...
function CalcRow({ eq, result, color }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 0", borderBottom: `1px solid ${T.border}`, fontSize: 11 }}>
      ...
      <span style={{ color: color || T.syn_main, fontWeight: 500, fontFamily: "monospace" }}>{result}</span>
    </div>
  );
}
...
function SolarCellCard({ mat, eg, method, recipe, eff, color, steps }) {
  ...
  return (
    <div style={{ padding: "8px 10px", cursor: "pointer" }} onClick={() => setExpanded(!expanded)}>
      ...
      <div style={{ fontSize: 10, color, fontWeight: 500, marginTop: 4 }}>Best η: {eff}</div>
      ...
      {expanded && steps && (
        <div style={{ marginTop: 8, borderTop: `1px solid ${color}20`, paddingTop: 8 }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ fontSize: 11, fontWeight: 500, color, marginBottom: 4 }}>{steps[activeStep].title}</div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <FAQAccordion title="What is CVD? (Chemical Vapor Deposition)" color={T.syn_cvd} isOpen={openItem === "cvd_what"} onClick={() => toggle("cvd_what")}>
        ...
        <div style={mb}>
          <span style={{ color: T.syn_cvd, fontWeight: 500 }}>General CVD reaction:</span><br />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
          {cvdVariants.map(v => (
            <div key={v.name} style={{ padding: "8px", fontSize: 10 }}>
              ...
              <div style={{ color: T.ink, fontWeight: 500, marginTop: 4 }}>{v.use}</div>
            </div>
          ))}
        </div>
      </FAQAccordion>
      ...
      <FAQAccordion title="Interactive: Step-by-Step CVD Process (click each step)" color={T.syn_main} isOpen={openItem === "cvd_steps"} onClick={() => toggle("cvd_steps")}>
        ...
        <svg width={420} height={260} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          {/* Chamber walls */}
          <rect x={40} y={40} width={340} height={180} rx={10} fill={T.bg} stroke={"color-mix(in srgb, var(--ink) 19%, transparent)"} strokeWidth={2} />
          {/* Substrate (heated stage) */}
          <rect x={100} y={185} width={220} height={20} rx={3} fill={T.syn_sol + "60"} stroke={T.syn_sol} strokeWidth={1.5} />
          <text x={210} y={198} textAnchor="middle" fill={T.syn_sol} fontSize={9} fontWeight={500}>Substrate ({cvdT}°C)</text>
          {/* Growing film */}
          ...
          <text x={32} y={105} textAnchor="middle" fill={T.syn_cvd} fontSize={7} fontWeight={500}>GAS IN</text>

          {/* Gas outlet */}
          <rect x={375} y={80} width={25} height={40} rx={3} fill={"color-mix(in srgb, var(--muted) 13%, transparent)"} stroke={T.muted} strokeWidth={1} />
          ...
          {cvdStep === 1 && <>
            ...
            <text x={210} y={70} textAnchor="middle" fill={T.syn_sol} fontSize={10} fontWeight={500}>Gas-phase decomposition</text>
          </>}
          ...
          {cvdStep === 4 && <>
            {Array.from({ length: 5 }, (_, i) => (
              <circle key={i} cx={150 + i * 30} cy={140 - (tick + i * 8) % 60} r={2.5} fill={"color-mix(in srgb, var(--muted) 50%, transparent)"}>
          ...
          <text x={210} y={248} textAnchor="middle" fill={T.muted} fontSize={9} fontWeight={500}>{step.title}</text>
        </svg>
      </FAQAccordion>
        ...
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 220 }}>
            ...
            <div style={{ marginTop: 8, fontSize: 11, color: T.muted, lineHeight: 1.6, background: T.syn_cvd + "08", borderRadius: 8, padding: "8px 12px" }}>
              {cvdT < 300 && <span style={{ color: T.syn_ald, fontWeight: 500 }}>Very low T — consider pecvd (plasma assists decomposition).</span>}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      <FAQAccordion title="Interactive: Step-by-Step PVD Process" color={T.syn_main} isOpen={openItem === "pvd_steps"} onClick={() => toggle("pvd_steps")}>
        ...
        <svg width={420} height={260} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          <rect x={15} y={15} width={390} height={230} rx={6} fill={T.surface} />
          {/* Vacuum chamber */}
          <rect x={40} y={30} width={340} height={200} rx={10} fill={T.bg} stroke={"color-mix(in srgb, var(--ink) 19%, transparent)"} strokeWidth={2} />
          {pvdStep === 0 && <text x={210} y={130} textAnchor="middle" fill={T.syn_pvd} fontSize={14} fontWeight={500}>Vacuum: 10⁻⁶ Torr</text>}

          {/* Source at bottom */}
          <rect x={160} y={200} width={100} height={20} rx={4} fill={T.syn_ald + "50"} stroke={T.syn_ald} strokeWidth={1.5} />
          <text x={210} y={214} textAnchor="middle" fill={T.syn_ald} fontSize={9} fontWeight={500}>Source</text>

          {/* Substrate at top */}
          <rect x={120} y={45} width={180} height={15} rx={3} fill={T.syn_main + "40"} stroke={T.syn_main} strokeWidth={1.5} />
          <text x={210} y={55} textAnchor="middle" fill={T.syn_main} fontSize={8} fontWeight={500}>Substrate</text>

          {/* Evaporation plume */}
          {pvdStep >= 1 && <>
            <text x={210} y={195} textAnchor="middle" fill={T.syn_ald} fontSize={10} fontWeight={500}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <FAQAccordion title="What is Sol-Gel?" color={T.syn_sol} isOpen={openItem === "sg_what"} onClick={() => toggle("sg_what")}>
        ...
        <div style={mb}>
          <span style={{ color: T.syn_sol, fontWeight: 500 }}>Sol-Gel chemistry:</span><br />
      ...
      <FAQAccordion title="Interactive: Step-by-Step Sol-Gel Process" color={T.syn_spin} isOpen={openItem === "sg_steps"} onClick={() => toggle("sg_steps")}>
        ...
        <svg width={420} height={250} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          {sgStep === 0 && <>
            ...
            <text x={210} y={55} textAnchor="middle" fill={T.syn_sol} fontSize={11} fontWeight={500}>Colloidal Sol</text>
          ...
          {sgStep === 1 && <>
            ...
            <text x={210} y={55} textAnchor="middle" fill={T.syn_hydro} fontSize={11} fontWeight={500}>Gel Network Forming</text>
          ...
          {sgStep === 3 && <>
            ...
            <text x={210} y={85} textAnchor="middle" fill={T.syn_ald} fontSize={11} fontWeight={500}>500°C — Crystallization</text>
            <text x={210} y={205} textAnchor="middle" fill={T.muted} fontSize={9}>Organics burn off, oxide crystallizes</text>
          </>}

          <text x={210} y={238} textAnchor="middle" fill={T.muted} fontSize={9} fontWeight={500}>{step.title}</text>
        </svg>
      </FAQAccordion>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <FAQAccordion title="What is ALD? (Atomic Layer Deposition)" color={T.syn_ald} isOpen={openItem === "ald_what"} onClick={() => toggle("ald_what")}>
        ...
        <div style={mb}>
          <span style={{ color: T.syn_ald, fontWeight: 500 }}>ALD of Al₂O₃ (most common ALD process):</span><br /><br />
      ...
      <FAQAccordion title="Interactive: ALD Cycle Animation (click each half-cycle)" color={T.syn_main} isOpen={openItem === "ald_steps"} onClick={() => toggle("ald_steps")}>
        ...
        <svg width={420} height={220} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          <text x={210} y={178} textAnchor="middle" fill={T.syn_main} fontSize={9} fontWeight={500}>Substrate</text>
          ...
          {aldStep === 1 && <>
            ...
            <text x={210} y={90} textAnchor="middle" fill={T.syn_cvd} fontSize={12} fontWeight={500}>N₂ PURGE →</text>
          ...
          {aldStep === 2 && <>
            ...
            <text x={210} y={45} textAnchor="middle" fill={T.syn_hydro} fontSize={11} fontWeight={500}>H₂O pulse</text>
            {Array.from({ length: 8 }, (_, i) => (
              <g key={i}>
                <line x1={85 + i * 35} y1={160} x2={85 + i * 35} y2={145} stroke={T.syn_ald} strokeWidth={2} />
                <circle cx={85 + i * 35} cy={142} r={4} fill={T.syn_ald + "80"} />
          ...
          {aldStep === 3 && <>
            <text x={210} y={45} textAnchor="middle" fill={T.syn_main} fontSize={12} fontWeight={500}>1 cycle complete → ~1 Å deposited</text>
          ...
          <text x={210} y={208} textAnchor="middle" fill={T.muted} fontSize={9} fontWeight={500}>{step.title}</text>
        </svg>
      </FAQAccordion>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <FAQAccordion title="What is MBE? (Molecular Beam Epitaxy)" color={T.syn_mbe} isOpen={openItem === "mbe_what"} onClick={() => toggle("mbe_what")}>
        ...
        <div style={mb}>
          <span style={{ color: T.syn_mbe, fontWeight: 500 }}>MBE characteristics:</span><br />
      ...
      <FAQAccordion title="Animated: MBE Growth Chamber" color={T.syn_main} isOpen={openItem === "mbe_anim"} onClick={() => toggle("mbe_anim")}>
        <svg width={420} height={280} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          <text x={210} y={35} textAnchor="middle" fill={T.syn_mbe} fontSize={11} fontWeight={500}>MBE Growth Chamber (uhv: 10⁻¹⁰ Torr)</text>
          ...
          <text x={210} y={60} textAnchor="middle" fill={T.syn_main} fontSize={8} fontWeight={500}>Substrate (600°C)</text>
          {/* Growing film */}
          ...
          {[
            ...
            <g key={ci}>
              ...
              <text x={cell.x} y={225} textAnchor="middle" fill={cell.color} fontSize={10} fontWeight={500}>{cell.label}</text>
              {/* Molecular beams */}
          ...
          <text x={30} y={85} fill={T.syn_mbe} fontSize={8} fontWeight={500}>RHEED</text>
          ...
          {[50, 150, 260, 350].map((x, i) => (
            <rect key={i} x={x - 12} y={193} width={24} height={4} rx={1} fill={i % 2 === tick % 4 ? T.syn_main : T.muted} stroke={"color-mix(in srgb, var(--ink) 19%, transparent)"} strokeWidth={0.5} />
          ))}
      ...
      <FAQAccordion title="Interactive: Step-by-Step Spin Coating Process" color={T.syn_main} isOpen={openItem === "sc_anim"} onClick={() => toggle("sc_anim")}>
        ...
        <svg width={420} height={260} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          {scStep === 0 && <>
            ...
            <text x={210} y={194} textAnchor="middle" fill={T.syn_sol} fontSize={9} fontWeight={500}>Substrate</text>
            {/* Pipette */}
            ...
            <text x={210} y={22} textAnchor="middle" fill={T.syn_spin} fontSize={8} fontWeight={500}>Pipette</text>
            {/* Falling droplet */}
            ...
            <text x={210} y={225} textAnchor="middle" fill={T.syn_spin} fontSize={10} fontWeight={500}>Dispense 20-100 μL onto center</text>
          </>}
          ...
          {scStep === 1 && <>
            ...
            {/* RPM indicator */}
            <text x={210} y={60} textAnchor="middle" fill={T.syn_spin} fontSize={16} fontWeight={500}>↻ Accelerating</text>
            ...
            <text x={210} y={225} textAnchor="middle" fill={T.syn_sol} fontSize={10} fontWeight={500}>Centrifugal force spreads solution outward</text>
          </>}
          ...
          {scStep === 2 && <>
            ...
            <text x={210} y={55} textAnchor="middle" fill={T.syn_main} fontSize={14} fontWeight={500}>4000 rpm — Steady State</text>
            ...
            <text x={210} y={225} textAnchor="middle" fill={T.syn_main} fontSize={10} fontWeight={500}>Viscous flow + evaporation thin the film</text>
          </>}
          ...
          {scStep === 3 && <>
            ...
            {/* Edge bead labels */}
            <text x={90} y={125} textAnchor="middle" fill={T.syn_cvd} fontSize={7} fontWeight={500}>Edge bead</text>
            <text x={330} y={125} textAnchor="middle" fill={T.syn_cvd} fontSize={7} fontWeight={500}>Edge bead</text>
            ...
            <text x={155} y={144} fill={T.syn_spin} fontSize={7} fontWeight={500}>~500 nm</text>
            {/* Heat from below */}
            ...
            <text x={210} y={55} textAnchor="middle" fill={T.syn_cvd} fontSize={13} fontWeight={500}>Drying & Solidification</text>
            ...
            <text x={210} y={225} textAnchor="middle" fill={T.syn_spin} fontSize={10} fontWeight={500}>Uniform film with edge bead at rim</text>
          </>}
        </svg>
      </FAQAccordion>
        ...
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 240 }}>
            ...
            <div style={{ marginTop: 10, fontSize: 11, color: T.muted, lineHeight: 1.6, background: T.syn_spin + "08", borderRadius: 8, padding: "8px 12px" }}>
              {thickness < 50 && <span style={{ color: T.syn_cvd, fontWeight: 500 }}>Very thin — may have pinholes. Consider multi-coat.</span>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <FAQAccordion title="What is Hydrothermal Synthesis?" color={T.syn_hydro} isOpen={openItem === "ht_what"} onClick={() => toggle("ht_what")}>
        ...
        <div style={mb}>
          <span style={{ color: T.syn_hydro, fontWeight: 500 }}>Hydrothermal conditions:</span><br />
          ...
          <span style={{ color: T.syn_hydro, fontWeight: 500 }}>Common products:</span><br />
      ...
      <FAQAccordion title="Interactive: Step-by-Step Hydrothermal Process" color={T.syn_main} isOpen={openItem === "ht_anim"} onClick={() => toggle("ht_anim")}>
        ...
        <svg width={420} height={260} style={{ background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, display: "block", margin: "0 auto", overflow: "hidden" }}>
          ...
          {htStep === 0 && <>
            ...
            <text x={210} y={55} textAnchor="middle" fill={T.syn_hydro} fontSize={11} fontWeight={500}>Precursor Solution</text>
            ...
            <rect x={185} y={182} width={50} height={8} rx={4} fill={"color-mix(in srgb, var(--muted) 31%, transparent)"} transform={`rotate(${tick*2%360},210,186)`}/>
            {/* Labels */}
            <text x={210} y={215} textAnchor="middle" fill={T.muted} fontSize={9}>Zn(NO₃)₂ + hmta in H₂O</text>
            <text x={210} y={228} textAnchor="middle" fill={T.syn_hydro} fontSize={9} fontWeight={500}>pH controlled, stirring</text>
          </>}

          {/* Step 2: Seal in Autoclave */}
          {htStep === 1 && <>
            {/* Outer steel jacket */}
            <rect x={120} y={45} width={180} height={170} rx={14} fill={"color-mix(in srgb, var(--muted) 8%, transparent)"} stroke={T.muted} strokeWidth={3}/>
            {/* Inner Teflon liner */}
            ...
            <rect x={125} y={40} width={170} height={18} rx={6} fill={"color-mix(in srgb, var(--muted) 19%, transparent)"} stroke={T.muted} strokeWidth={2}/>
            {/* Bolts */}
            {Array.from({length:4},(_,i)=><circle key={i+8} cx={145+i*42} cy={49} r={5} fill={"color-mix(in srgb, var(--muted) 25%, transparent)"} stroke={T.muted} strokeWidth={1.5}/>)}
            {/* Labels */}
            <text x={210} y={36} textAnchor="middle" fill={T.muted} fontSize={10} fontWeight={500}>Stainless Steel Cap</text>
            ...
            <text x={210} y={228} textAnchor="middle" fill={T.syn_sol} fontSize={10} fontWeight={500}>Fill 60-80% — leave room for steam</text>
          </>}
          ...
          {htStep === 2 && <>
            ...
            <text x={210} y={42} textAnchor="middle" fill={T.syn_pvd} fontSize={14} fontWeight={500}>200°C — 16 atm</text>
            <text x={210} y={228} textAnchor="middle" fill={T.syn_pvd} fontSize={10} fontWeight={500}>Superheated water dissolves metal oxides</text>
          </>}
          ...
          {htStep === 3 && <>
            ...
            <text x={210} y={38} textAnchor="middle" fill={T.syn_main} fontSize={12} fontWeight={500}>Crystal Growth — Layer by Layer</text>
            <text x={210} y={215} textAnchor="middle" fill={T.muted} fontSize={9}>Slow growth (2-48 hrs) → well-faceted crystals</text>
            <text x={210} y={228} textAnchor="middle" fill={T.syn_main} fontSize={10} fontWeight={500}>Atoms add to crystal faces from solution</text>
          </>}
          ...
          {htStep === 4 && <>
            ...
            <text x={100} y={55} textAnchor="middle" fill={T.syn_ald} fontSize={9} fontWeight={500}>Cool Down</text>
            {/* Cooling arrows */}
            ...
            <text x={215} y={55} textAnchor="middle" fill={T.syn_hydro} fontSize={9} fontWeight={500}>Wash</text>
            ...
            <text x={325} y={55} textAnchor="middle" fill={T.syn_main} fontSize={9} fontWeight={500}>Harvest</text>
            {Array.from({length:4},(_,i)=>{
              const cx = 300+i*18;
              const cy = 120;
            ...
            <text x={210} y={228} textAnchor="middle" fill={T.syn_ald} fontSize={10} fontWeight={500}>High-quality nanocrystals ready for use</text>
          </>}
        </svg>
      </FAQAccordion>
            ...
            <thead>
              <tr style={{ borderBottom: `2px solid ${T.syn_main}30` }}>
                {["Method", "Vacuum?", "Temp", "Rate", "Thickness Control", "Conformality", "Cost", "Best For"].map(h => (
                  <th key={h} style={{ padding: "6px 8px", textAlign: "left", color: T.syn_main, letterSpacing: 0.5, textTransform: "none", fontWeight: 500, fontSize: 9 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={method} style={{ background: i % 2 === 0 ? T.syn_main + "05" : "transparent", borderBottom: `1px solid ${T.border}55` }}>
                  <td style={{ padding: "6px 8px", fontWeight: 500, color: T.ink }}>{method}</td>
                  ...
                  <td style={{ padding: "6px 8px", color: T.ink, fontWeight: 500 }}>{ctrl}</td>
                  <td style={{ padding: "6px 8px", color: T.muted }}>{conf}</td>
                  <td style={{ padding: "6px 8px", color: T.muted }}>{cost}</td>
                  <td style={{ padding: "6px 8px", color: T.syn_main, fontWeight: 500 }}>{best}</td>
                </tr>
              ))}
            </tbody>
          </table>
